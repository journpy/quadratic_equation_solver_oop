...
class QuadraticFunctions:
	...
	def quadratic_formula(self):
		...
		if d >= 0:
			x_1, x_2 = (-self.b + math.sqrt(d))/(2*self.a), (-self.b - math.sqrt(d))/(2*self.a)

		# If discriminant is negative, it will have complex roots and will 
			# require python's complex math library.
		else:
			x_1, x_2 = (-self.b + cmath.sqrt(d))/(2*self.a), (-self.b - cmath.sqrt(d))/(2*self.a)
	...
	def plot_and_display_graph(self):
		...
		qf = QuadraticFunctions(self.a, self.b, self.c)
		qf.style_graph()
		plt.title(f"Graph of {self.a}x^2 + {self.b}x + {self.c}.", fontsize=12, fontweight='bold', c='r')
		plt.plot(x, y, 'b')
		plt.show()
